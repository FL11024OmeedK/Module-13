...
> **Status:** the mobile app is in active development. Project setup, dependencies, assets, the API/tunnel connection, the three-level navigation structure (root Stack → customer Tabs → restaurant Stack), the shared header/footer, the Login page, the Restaurant List page (with rating/price filters), the Restaurant Menu page, and the Order Confirmation modal (Processing/Success/Error states) are complete; the success path is verified on-device, the error-state path is implemented but not yet exercised on-device. The Order History screens are being built next, each with its own spec in `ai/features/`.
